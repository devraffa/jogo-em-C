...
//Esse tipo � um array de dois inteiros.
struct coordenadas{
    int coor[2];
};
//essa fun�ao do tipo struct recebe um string e retorna um vetor de dois inteiros
...
//verificar se peca se moveu na horznt, vertcl ou diagns
    ...
    else return 0;                      //movimento inv�lido
//verificar se casa de destino est� vazia
    if(naCasa(cood, matriz) != ' ')return 0;    //movimento inv�lido
...
//verifica vit�ria nas diagon secund
    //essa parte s� pega "metade do tabuleiro"
    for (i = 4; i < 9; i++){
        for(a = i, b = 0; a >= 4; a--, b++){
            contx = 0, conto = 0;
...
//verifica vit�ria nas diagon princp
    //essa parte s� pega metade do tabuleiro
    for (i = 4; i < 9; i++){
        for (a = i, b = 8; a >= 4; a--, b--){
            contx = 0, conto = 0;
...
int main(){
    ...
    printf("\n     --------------------------------------------------");
	printf("\n     Jogador X - aliados *  (o jogador X come�a o jogo)");
    printf("\n     Jogador O - aliados �");
    printf("\n     --------------------------------------------------");
    printf("\n\n     APERTE ENTER PRA INICIAR");
    setbuf(stdin, NULL);
    scanf("%c", &enter);
//  Para gerar aleatoriamente as primeiras pe�as
    srand(time(NULL));
    for (i = 0 ; i < 9 ; i++)
        for (j = 0 ; j < 9 ; j++)
                matriz[i][j] = ' ';
    ...
    while (1){
        ...
        while(1){
            ...
            if(validaPeca(coop, naCasa(coop, matriz), peca, matriz) == 3){
                printf("\n     SELECIONE UMA PE�A %c QUE POSSA SE MOVIMENTAR\n", peca);
                printf("\n     ENTER PRA CONTINUAR");
                setbuf(stdin, NULL);
                scanf("%c", &enter);
            }else{
                printf("\n     SELECIONE UMA CASA QUE POSSUA UMA PE�A %c!\n", peca);
                printf("\n     ENTER PRA CONTINUAR");
                setbuf(stdin, NULL);
                scanf("%c", &enter);
            }}
        while(1){
            ...
            exibeTabuleiro(matriz, turno, peca);
            printf("\n     MOVIMENTO INV�LIDO! ESCOLHA UM DESTINO POSS�VEL!\n");
            printf("\n     ENTER PRA CONTINUAR");
            setbuf(stdin, NULL);
            scanf("%c", &enter);
        }
